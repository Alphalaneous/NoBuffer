...
class $modify(MyGJBaseGameLayer, GJBaseGameLayer) {
    ...
    void update(float dt) {
        ...
        if (!m_player1->m_isShip && !m_player1->m_isDart && !m_player1->m_isRobot && !fields->m_wasOnGroundP1) {
            GJBaseGameLayer::handleButton(false, 1, true);
        }

        if (m_level->m_twoPlayerMode) {
            if (!m_player2->m_isShip && !m_player2->m_isDart && !m_player2->m_isRobot && !fields->m_wasOnGroundP2) {
